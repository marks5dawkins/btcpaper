...
class Modelo_Arima():
  ...
  def Metricas(self):
    ...
    rows = []
    MAE = metrics.mean_absolute_error(self.__test, self.__forc[self.__name])
    MAPE = metrics.mean_absolute_percentage_error(self.__test, self.__forc[self.__name])
    MSE = metrics.mean_squared_error(self.__test, self.__forc[self.__name])
    RMSE = np.sqrt(metrics.mean_absolute_error(self.__test, self.__forc[self.__name]))   
